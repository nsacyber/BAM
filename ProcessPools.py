...
class PSFXMgr(CabMgr):
    ...
    def __init__(self, pdir, dest, poolsize, pemgr, db, local, globqueue, basedir):
        super(PSFXMgr, self).__init__(pdir, dest, poolsize, pemgr, db, local, globqueue)
        self.jobs = queue.Queue()
    ...
    def PSFXExtract(cls, src, dest, basedir):
        ...
        firststagingdir = Path(os.join(os.getcwd(),"firsStage"))
        reversedir = Path(os.join(os.getcwd(),"hydrateWithReverse"))
        forwarddir = Path(os.join(os.getcwd(),"hydrateWithForward"))
        ...
        for line in stroutput:
            ...
            if word.endswith(".cab"):
                ...
                childdir = Path(os.path.join(firststagingdir, word))
        ...
        for root, _, file, in os.walk(firststagingdir):
            if not file:
                continue
            base = Path(os.path.join(basedir, file))
            currentFilePath = Path(os.path.join(root, file))
            # if we're given null patch file, just apply it directly and be done
            if "\\n" in root:
                finalFilePath = Path(os.path.join(dest, file))
            ...
            # if we have a forward or reverse patch, move them to the respective staging folder
                ...
                forwardFilePath = Path(os.path.join(forwarddir, file))
                os.rename(currentFilePath, forwardFilePath)
            elif "\\r" in root:
                reverseFilePath = Path(os.path.join(reversedir, file))
        ...
        for _, _, file in os.walk(reversedir):
            revpatch = Path(os.path.join(reversedir, file))
            forpatch = Path(os.path.join(forwarddir, file))
            # naively assume that everything is just at the base level of directory for now
            baseFile = Path(os.path.join(basedir,file))
            finalFile = Path(os.path.join(dest, file))
