...
class CabMgr(threading.Thread):
    ...
    '''
    def __init__(self, pdir: str, dest: str, poolsize: int, pemgr, psfxmgr, db, local: bool, globqueue: queue.Queue, restore_file:str=None):
        super(CabMgr, self).__init__()

        if restore_file:
            with open(restore_file, 'rb') as f:
                self.jobs = dill.load(f)
        else:
    ...
    def passresult(self, future: asyncio.Future):
        ...
        pemgr or psfxmgr
        '''
        # if exception generated by extracttask, notify here. If not, get result and send to pemgr
        self.in_progress.pop(future)
        ...
        if fexception is KeyboardInterrupt:
            ...
            job = future.result()
            if job is not None:
                # if the directory contains ncabdir, then we know that it is already a nestedCab, so remove it from deliverables, otherwise, send to DBMgr
                if job[0] == "PSFX":
                    version = int(job[6])
                    for j in job[2]:
        ...
        # if there are no more nested cabs, set the jobs incoming event since there is at least 1 more job item to allow thread to complete execution
        self.workremaining -= 1
    ...
    def run(self):
        ...
        start = time()
        count = 0
        if not os.path.isdir(".\\restore_files"):
            os.mkdir(".\\restore_files")
        restore_file = ".\\restore_files\\restore_cab"
        ...
        with ProcessPoolExecutor(max_workers=self.poolsize, initializer=wkr_logconfig, initargs=(self.globqueue, _mgrlogger)) as executor:
            while not self.jobs.empty():                
                ...
                jobitem = self.jobs.get()
                if self.localaction:
                    future = executor.submit(self.dbupdate, jobitem, \
                                                self.dest)
                else:
                    # arbitrary point at which to take a snapshot of the remaning queue.
                    if count % 100 == 0:
                        with open(restore_file, 'wb') as f:
                            for value in self.in_progress.values():
                                self.jobs.put(value)
                            dill.dump(self.jobs, f)
                    count += 1
                    future = executor.submit(self.extracttask, jobitem, \
                                                self.dest)
                    self.in_progress.update({future: jobitem})
    ...
    def dbupdate(cls, src: str, pdestdir: str) -> Tuple | None:
        ...
        # No need to locate nested CABs/MSUs as long the parent update file is found. Revisit if needed
    ...
    def extracttask(cls, src: str, dst: str) -> Tuple | None:
        ...
        # assuming that Microsoft is not devious enough to put a cab in a exe proceed to extract cab/msu contents
        cabfile = src
        ...
        while not cabqueue.empty():
            ...
            # found that mum files in psfx cabs will have a psfx package format attribute in the xml.
            ...
            lines = listing.split("\r\n")
            for line in lines:
                # if a line contains a PE file, then we've reached the end of a branch, so break and go to the next cab.
...
class PSFXMgr(CabMgr):
    ...
    '''
    def __init__(self, pdir: str, dest: str, poolsize: int, pemgr, db, local: bool, globqueue: queue.Queue, basedir: str, restore_file:str=None):
    ...
    def run(self):
        ...
        if not os.path.isdir(".\\restore_files"):
            os.mkdir(".\\restore_files")
        restore_file = ".\\restore_files\\restore_psfx"
        
        global _mgrlogger
        futures = []
        with ProcessPoolExecutor(max_workers=self.poolsize, initializer=wkr_logconfig, initargs=(self.globqueue, _mgrlogger)) as executor:
            while self.cabmgrRunning.is_set() or not self.jobs.empty():
                # arbitrary point to create a checkpoint
                if count % 100 == 0:
                    with open(restore_file, 'wb') as f:
                        # ensure that the in progress jobs are added back to the jobs queue so that they are also pickled and restarted later
                        for value in self.in_progress.values():
                            self.jobs.put(value)
                        dill.dump(self.jobs, f)
                count += 1
                ...
                try:
                    ...
                    # this is done because we know that we'll get instances of queue being empty but since we may still want to wait for other items to come in while the previous manager is running, we need to ignore the queue being empty. Previously we used the regular get which blocked for waiting for another item, but this ended up causing the thread to hang because the loop could not check the event that said the previous manager had completed.
                    continue
                psfxcab = tuple[0]
                version = int(tuple[1])
                dest = "\\".join(psfxcab.split("\\")[0:-1])
                future = executor.submit(self.PSFXExtract, psfxcab, dest, self.index, version, UpdateSha)
                self.in_progress.update({future: item})
                future.add_done_callback(self.passresult)
    ...
    def PSFXExtract(cls, src: str, dest: str, index: dict, version: int, UpdateSha: str) -> Tuple:
        ...
        full_product_regex = re.compile(r"([^\s\\])+_[\da-f]{16}_\d{1,2}\.[01]\.\d{1,5}\.\d+_\S+_[\da-f]{16}")
        chaff_regex = re.compile(r"_[\da-f]{16}")
    ...
    def addjob(self, psfxcab: str, version: int, updateid: str):
        # just add the cab to the queue.
        jobtuple = (psfxcab, version, updateid)
        self.jobs.put(jobtuple)
...
class PEMgr(threading.Thread):
    ...
    db - DBMgr for which to refer database write jobs to
    '''
    def __init__(self, poolsize: int, symmgr, db, globqueue: queue.Queue, restore_file:str=None):
        super(PEMgr, self).__init__()
        self.poolsize = poolsize
        self.symmgr = symmgr
        self.dbc = db
        self.globqueue = globqueue
        # jobs - holds the paths to the directories containing binaries to sort and clean up
        if restore_file:
            with open(restore_file, 'rb') as f:
                self.jobs = dill.load(f)
        else:
        ...
        if fexception is KeyboardInterrupt:
            ...
            if result is not None:
                if result[3]["builtwithdbginfo"] and self.symmgr is not None:
                    # only need to use verify if PE was builtwithdbginfo here because that condition takes precedence over the stripped condition and if the item is stripped, a check must be made by symchk anyway to find the .dbg file. Reference: https://docs.microsoft.com/en-us/windows-hardware/drivers/debugger/symchk-command-line-options in the DBG file options.
                    self.symmgr.addjob(str(result[0][0]), result[1], result[2], result[0][1])
    ...
    def run(self):
        ...
        '''
        count = 0
        restore_file = ".\\restore_files\\restore_pe"
        self.cabmgrRunning.wait()
            ...
            while not self.jobs.empty() or self.psfxmgrRunning.is_set() or self.cabmgrRunning.is_set():
                ...
                for root, _, files in os.walk(jobdir):
                    for file in files:
                        ...
                        # file naming convention in wsuscontent folder seems to be the same as whatever is at the end of update file obtained from update catalog so for our current purposes, searching via this regex is fine, but this format doesn't match with the UpdateID column in the database so correlation will have to be done in a different way? Using sha256 hash of file name for now, but need to figure something else out eventually since we may want to be able to correlate to the WSUS DB eventually. That would mean I have to find out how to get the real Update ID from somewhere.
    ...
    def petask(cls, jobfile: str, updateid: str) -> Tuple | None:
        ...
        if ispe(jobfile):
            ...
            if wsuse_db.dbentryexistwithsymbols(globs.DBCONN.cursor(),     \
                ...
                # if PE is already in db with symbols obtained, do not retask job to symbol manager, return None instead
                return results
            else:
                pass
            ...
            # getting to this point means item is not in db, may need to come up with case where db needs to update item though
            infolist = {
            ...
            direntires=[ pefile.DIRECTORY_ENTRY['IMAGE_DIRECTORY_ENTRY_DEBUG'], pefile.DIRECTORY_ENTRY['IMAGE_DIRECTORY_ENTRY_RESOURCE'] ]
            ...
            # a PE only have 1 VERSIONINFO, but multiple language strings More information on different properites can be found at https://msdn.microsoft.com/en-us/library/windows/desktop/aa381058 https://msdn.microsoft.com/en-us/library/windows/desktop/aa381049
                                        ...
                                        for field, value in strtable.entries.items():
                                            ...
                                            if dfield == "SpecialBuild":
                                                infolist["SpecialBuild"] \
                                                    = dvalue
            # Get the OS this PE is designed towards. Microsoft PE files distributed via Microsoft's Updates typically use the ProductVersion file properties to indicate the OS a specific PE file is built towards. If this is a Microsoft binary, the Product version is typically the OS version it was built towards, but other products this is not necessarily true
...
class SymMgr(threading.Thread):
    ...
    '''
    def __init__(self, poolsize: int, symserver: str, symdest: str, db, symlocal:bool=False, globqueue:queue.Queue=None, restore_file:str=None):
        ...
        if restore_file:
            ...
            self.jobs = queue.Queue()
        #  - an Event that indicates to SymMgr when jobs are ready for it to process
    ...
    def addjob(self, jobfile, hash1, hash2, updateid):
        '''
        receive jobs from PEMgr, add those jobs to jobs List, and set
        the  Event to indicate there are jobs waiting to be
        processed
        '''
        jobset = (jobfile, hash1, hash2, updateid)
    ...
    def run(self):
        ...
        '''
        count = 0
        restore_file = ".\\restore_files\\restore_sym"
        self.peRunning.wait()
        start_time = time()
        # setup workers and Executor
        global _mgrlogger
        ...
        with ProcessPoolExecutor(max_workers=self.poolsize, initializer=wkr_logconfig, initargs=(self.globqueue, _mgrlogger)) as executor:
            ...
            while not self.jobs.empty() or self.peRunning.is_set():
                ...
                self.in_progress.update({future: job})
    ...
    def symtask(cls, jobitem: Tuple, symserver: str, symdest: str, symlocal: bool) -> Tuple | None:
        ...
        try:
            with subprocess.Popen(args, shell=False, stdout=subprocess.PIPE, stderr=subprocess.PIPE) \
                    as psymchk:
                # communicate used as symchk's output is for one file and is not "large or unlimited"
...
class DBMgr(threading.Thread):
    ...
    def __init__(self, exdest: str, dbconn:sqlite3.Connection=None):
        ...
        self.userinterrupt = threading.Event()
        # donecount - used to indicate when there are no more jobs being submit to the DBMgr by any of the other 3 Mgrs. When count is 3, indicates that there are no more jobs
        self.donecount = 0
        self.dbrecordscnt = 0
        self.exdest = exdest
    ...
    def run(self):
        ...
        while self.donecount < 3:
            ...
            while not self.jobqueue.empty():
                ...
                # For every 5k queries, end transaction, commit, then restart transaction
