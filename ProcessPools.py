...
class CabMgr(threading.Thread):
    ...
    '''
    def __init__(self, pdir, dest, poolsize, pemgr, psfxmgr, db, local, globqueue):
    ...
    def passresult(self, future):
        ...
        if fexception is not None:
            ...
            if job is not None:
                ...
                if job[0] == "PSFX":
                    self.psfxmgr.addjob(job[1])

                ncabdir = self.pdir + "\\nestedCabs"
                

                if self.pemgr is not None:
                    self.pemgr.receivejobset(job[0][0])
        ...
        if fexception is not None:
            ...
            if job is not None:
                ...
                if ncabdir in job[0][0]:
                            self.cabmgrlogger.log(logging.DEBUG, "[CABMGR] found nested cab, not adding to database")
                else:
    ...
    def requeuetask(self, future):
        ...
        if job is not None:
            if job[0][1]:
                for task in job[0][1]:
                    self.cabmgrlogger.log(logging.DEBUG, "[CABMGR] requeueing task: " + str(task))
                    self.addq(task)
    ...
    def run(self):
        ...
        # don't wait for results to pile up, task workers and send off to pemgr
        ...
        with ProcessPoolExecutor(max_workers=self.poolsize, initializer=wkr_logconfig, initargs=(self.globqueue, _mgrlogger)) as executor:
            while self.workremaining > 0:
                if not self.jobs.empty():
                    self.cabmgrlogger.log(logging.DEBUG, "[CABMGR] waiting for more extraction jobs. Current jobs:"    \
                ...
                while not self.jobs.empty():
                    ...
                    if self.localaction:
                        ...
                        future = executor.submit(self.extracttask, self.jobs.get(), \
                                                 self.pdir, self.dest)
                    self.cabmgrlogger.log(logging.DEBUG, "[CABMGR] jobs left: " + str(len(self.jobs)) + " " + \
        ...
        self.cabmgrlogger.log(logging.INFO, "[CABMGR] **************part 1 done**********************")
        print("[CABMGR] **************part 1 done**********************")
    ...
    @classmethod
    def extracttask(cls, src, dst):
        '''
        task for workers to extract contents of .cab file and return
        directory of result to for use by pemgr
        '''
        cablogger = logging.getLogger("BAM.Pools.cabwkr")
        ...
        logmsg = "[CABMGR] started on " + str(src) + " extracting files to " + str(dst)
        cablogger.log(logging.DEBUG, logmsg)
        ...
        if ispe(src):
            ...
            if not entryexists and cls.perform7zextract(src, newdir, cablogger) is None:
                return deliverables

            deliverables = ((str(newdir), []), hashes[0], hashes[1])
            ...
            if not validatecab(src):
                logmsg = "[CABMGR] {-} invalid file: " + src
                cablogger.log(logging.ERROR, logmsg)
                return None
            ...
            if not entryexists:
                ...
                cls.performcabextract("*.dll", src, newdir, cablogger)
                cls.performcabextract("*.exe", src, newdir, cablogger)
                cls.performcabextract("*.sys", src, newdir, cablogger)
            ...
            # again
            if not entryexists:
                listing = cls.performcablisting(src, cablogger)
                ...
                # assuming that PSFX type updates are not included with older style updates, so shouldn't need to check
                ...
                for line in stroutput:
                    if line.endswith(".cab") or line.endswith(".msu"):
                        ...
                        if not os.path.exists(ncabdir):
                            try:
                                os.mkdir(ncabdir)
                                ncabdir = str(Path(ncabdir).resolve())

                            except OSError as error:
                                logmsg = "[CABMGR] {-} unable to make nested cab directory: " + str(error)
                                cablogger.log(logging.ERROR, logmsg)
                                break

                        logmsg = "[CABMGR] beginning extraction of nestedcab: " + str(src)
                        cablogger.log(logging.DEBUG, logmsg)
                        extractstdout = cls.performcabextract(potentialfile, src, str(ncabdir), cablogger)

                        if extractstdout is not None:
                            # Case where there exists nested cabs with a .manifest file
                            newpath = None
                            for root, _, cabs in os.walk(ncabdir):
                                for cab in cabs:
                                    if str(cab) == potentialfile:
                                        newpath = str(Path(os.path.join(root, cab)).resolve())
                                        break
                            ...
                            # in at this point
                            if not validatecab(newpath):
                                logmsg = "[CABMGR] {-} extracttask: " + str(newpath) + " extracted from " + str(src) +  " is not a validate cab"
                                cablogger.log(logging.ERROR, logmsg)

                                logmsg = "[CABMGR] extracttask: Removing " + str(newpath)
                                cablogger.log(logging.ERROR, logmsg)
                            ...
                            logmsg = "[CABMGR] Creating " + str(newpath) + " for new thread..."
                            cablogger.log(logging.DEBUG, logmsg)

                            # return new location of extracted cab for addition to job queue
                            deliverables[0][1].append(str(newpath))

        logmsg = "[CABMGR] Extraction task completed for " + src
        cablogger.log(logging.DEBUG, logmsg)

        # deliverables should return the directory of the extracted materials, the directory(s) of additional cabs,
        # the sha256, and sha512 hashes of the original cab file.
        return deliverables
...
class PSFXMgr(CabMgr):
    ...
    '''
    def __init__(self, pdir, dest, poolsize, pemgr, db, local, globqueue, basedir):
        super(PSFXMgr, self).__init__(pdir, dest, poolsize, pemgr, self, db, local, globqueue)
        self.jobs = queue.Queue()
        self.jobReady = threading.Event()
        self.alldone = False
    ...
    def run(self):
        ...
        with ProcessPoolExecutor(max_workers=self.poolsize, initializer=wkr_logconfig, initargs=(self.globqueue, _mgrlogger)) as executor:
            ...
            while (not psfxcab == "Done"):
                future = executor.submit(self.PSFXExtract, psfxcab, self.dest, self.basedir)
                self.passresult(future)


    @classmethod
    def PSFXExtract(cls, src, dest, basedir):
        # extract files from all the cabs, backwards hydrate them, then forwards hyrdate them. finally return to PEMggr
        cabcontents = cls.performcablisting(src, None)
        if cabcontents is None:
            return None
        
        stroutput = cabcontents.decode("ascii").split("\r\n")

        # intermediate directories to hold all the stuff to be hydrated:
        firststagingdir = str(Path(os.join(os.getcwd(),"firsStage")))
        reversedir = str(Path(os.join(os.getcwd(),"hydrateWithReverse")))
        forwarddir = str(Path(os.join(os.getcwd(),"hydrateWithForward")))
        os.mkdir(firststagingdir)
        os.mkdir(reversedir)
        os.mkdir(forwarddir)
        ...
        for line in stroutput:
            word = line.split(' ')[-1]
            if word.endswith(".cab"):
                cls.performcabextract(word, src, dest)

                # extract from the just expanded cab now
                childdir = str(Path(os.path.join(firststagingdir, word)))
                cls.performcabextract("*", childdir, firststagingdir)

                rmfile(childdir)

        # find and sort the forwards and reverses into lists
        for root, _, file, in os.walk(firststagingdir):
            if not file:
                continue
            base = str(Path(os.path.join(basedir, file)))
            currentFilePath = str(Path(os.path.join(root, file)))
            # if we're given null patch file, just apply it directly and be done
            if "\\n" in root:
                finalFilePath = str(Path(os.path.join(dest, file)))
                MSDelta_imp.patch_binary(finalFilePath, currentFilePath)
            # if we have a forward or reverse patch, move them to the respective staging folder
            elif "\\f" in root:
                forwardFilePath = str(Path(os.path.join(forwarddir, file)))
                os.rename(currentFilePath, forwardFilePath)
            elif "\\r" in root:
                reverseFilePath = str(Path(os.path.join(reversedir, file)))
                os.rename(currentFilePath, reverseFilePath)
        
        # now that everything is moved, perform reverse and forward patching
        for _, _, file in os.walk(reversedir):
            revpatch = str(Path(os.path.join(reversedir, file)))
            forpatch = str(Path(os.path.join(forwarddir, file)))
            # naively assume that everything is just at the base level of directory for now
            baseFile = str(Path(os.path.join(basedir,file)))
            finalFile = str(Path(os.path.join(dest, file)))
            MSDelta_imp.patch_binary(baseFile, forpatch, revpatch, finalFile)

        # cleanup after all patches are applied
        shutil.rmtree(firststagingdir)
        shutil.rmtree(reversedir)
        shutil.rmtree(forwarddir)
    ...
    def passresult(self, future):
        return super().passresult(future)
...
class PEMgr(threading.Thread):
    ...
    def __init__(self, poolsize, symmgr, db, globqueue):
        ...
        # clean up
        self.jobs = queue.Queue()
        # jobsready is an event to indicate that CabMgr has job items ready
        self.jobsready = threading.Event()
        # alldone is a signal to indicate that CabMgr has completed and will not send
        # any more jobs over
        self.alldone = False
        self.pemgrlogger = logging.getLogger("BAM.Pools.PEMgr")

    def receivejobset(self, pejobs):
        '''
    ...
    def run(self):
        ...
        '''
        self.pemgrlogger.log(logging.DEBUG, "[PEMGR] ClnMgr starting")
        start_time = time()
        ...
        with ProcessPoolExecutor(max_workers=self.poolsize, initializer=wkr_logconfig, initargs=(self.globqueue, _mgrlogger)) as executor:
            while not self.alldone:
                if not self.jobs:
                    self.pemgrlogger.log(logging.DEBUG, "[PEMGR] waiting for more pe jobs")
                    self.jobsready.wait()
                ...
                self.pemgrlogger.log(logging.DEBUG, "[PEMGR] items left in PEMgr queue: " + str(len(self.jobs)))
            # ensure that executor and all futures shut down properly
            executor.shutdown(wait=True)
    ...
    @classmethod
    def petask(cls, jobfile, updateid):
        '''
...
class SymMgr(threading.Thread):
    ...
    def __init__(self, poolsize, symserver, symdest, db, symlocal=False, globqueue=None):
        ...
        # for it to process
        self.jobsready = threading.Event()
        # alldone - a signal that indicates to SymMgr when PEMgr has completed
